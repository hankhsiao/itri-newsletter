...
const TEMPLATE_FILE = 'template.htm';
const OUTPUT_HTML_FILE = 'itri.htm';
...
function execute(templateFile, outputFile) {
  let templateHTML = fs.readFileSync(templateFile).toString();
  ...
  async.auto({
    ...
    fs.writeFileSync(OUTPUT_HTML_FILE, output);
  });
}

execute(TEMPLATE_FILE, OUTPUT_HTML_FILE);
