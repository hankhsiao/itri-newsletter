...
function parseTags(item) {
  if (item.tags && item.tags.length > 0) {
    item.tags = item.tags.split(',').map(
      tag => {
        return {code: tag[0], code2: tag.split('-')[0], code3: tag.split('.')[0], name: tag};
      }
    );
  } else {
    item.tags = [];
...
function processData(results, isRelease) {
  const general = results.general[0];
  const techCategories = results.techCategories.map(
    item => {
      item.xn = item.xn.split(',').map(item => {
        return {code: item.split('.')[0], name: item};
      });
      return item;
    });
  let conference = results.conference.map(generateID);
  const confCategories = ['Virtual Conference', 'Physical Conference'];
  ...
  technology = _.groupBy(_.sortBy(results.technology, item => item.tags.length > 0 ? item.tags[0].name : 'z'), 'category');
